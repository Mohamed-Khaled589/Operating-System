...
//			cprintf("last element =%x\n",e->page_last_WS_element);
			...
			frame_info = get_frame_info(e->env_page_directory,(uint32) (virtual_address+ (i * PAGE_SIZE)), &ptr_page_table);
...
//			 cprintf("last element =%x\n",e->page_last_WS_element);
			 if(element!=e->page_last_WS_element){
					...
					LIST_INSERT_TAIL(&e->page_WS_list, element);
			 }
			 else
			 {
			 ...
			 if(counter==e->page_WS_list.size)
			 {
				 break;
			 }
		}
