...
int test_initial_alloc(int ALLOC_STRATEGY)
{
	...
	for (int i = 0; i < numOfAllocs; ++i)
	{
		for (int j = 0; j < allocCntPerSize; ++j)
		{
			...
			//Check returned va
			//cprintf("va = %x and curva= %x",va,curVA);
...
void test_free_block_FF()
{
	...
	*(endVAs[idx]) = idx ;
	...
	is_correct = 1;
	cprintf("address = %x",startVAs[numOfAllocs*allocCntPerSize]);
	cprintf("size = %d",get_block_size(startVAs[numOfAllocs*allocCntPerSize]));
	//Free last block
	free_block(startVAs[numOfAllocs*allocCntPerSize]);
	cprintf("size = %d",is_free_block(startVAs[numOfAllocs*allocCntPerSize]));
