...
int process_command(int number_of_arguments, char** arguments)
{
	...
	int num = LIST_SIZE(&foundCommands);
	if(num != 0)
	{
		for(int i = 0; i < num; i++) {
			...
			if(i == num-1)
				...
				LIST_REMOVE(&foundCommands,foundCommands.lh_first);
		}
	}
	for (int i = 0; i < NUM_OF_COMMANDS; i++)
		{
			if ((strcmp(arguments[0], commands[i].name) == 0) && ((actual_number_of_arguments == commands[i].num_of_args) || (commands[i].num_of_args == -1)))
			{
				return i;
			}
			...
			char *names = commands[i].name;
			for(int j = 0; j <= strlen(arguments[0]); j++)
			{

				if(strchr(names,arguments[0][j]) != 0)
				{
					names = strchr(names,arguments[0][j]);
					number_of_checks++;
				}
					/* the problem where that the linked list don't empty
					 * and that will show the last search you made
					 * try to search for command twice and see what will happen*/

			}
			if(number_of_checks == strlen(arguments[0]))
			{
				LIST_INSERT_HEAD(&foundCommands, &commands[i]);
			}
