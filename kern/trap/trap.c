...
//	cprintf("fault virtual address = %x\n");
	...
	else
	{
		if (userTrap)
		{
			...
			if(fault_va>=(curenv->limitt+PAGE_SIZE) && fault_va < USER_HEAP_MAX)
			{
				if((page_permissions &PERM_MARKED)!=PERM_MARKED)
				{
			...
			{
					if((page_permissions & PERM_WRITEABLE)!=PERM_WRITEABLE)
					{
						sched_kill_env(faulted_env->env_id);
					}
			}
