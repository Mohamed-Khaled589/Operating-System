...
{
	return allocate_user_mem(curenv, virtual_address, size);

}
void sys_env_set_nice(int nice_value)
{
	return env_set_nice(curenv,nice_value);
}
...
static int32 sys_get_hard_limit()
{
	return curenv->limitt;
}
//2017
...
void* sys_sbrk(int increment)
{
	...
	if(increment>0)
	{
//		cprintf("i\ahmed mohsen bbbbrbrbrbrb\n");
//		cprintf("size = %x\n",increment);
		if(env->brkk+ new_increment > env->limitt)
		{
			return (void*)-1;
		}
			env->brkk = env->brkk+new_increment;

		return (void*)(env->brkk - new_increment);
	}
	else if(increment < 0)
	{
		cprintf("from sys sbrk\n");
		if(env->brkk -increment < env->startt)
		{
			return (void*) -1;
		}
		env->brkk = env->brkk -increment;
		return (void*)env->brkk;
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	void params_validation(uint32 virtual_address, uint32 size)
		{
			unsigned char*ptr_v=(unsigned char*)(virtual_address);
			unsigned char*ptr_u=(unsigned char*)(USER_LIMIT);
			if(ptr_v==NULL)sched_kill_env(curenv->env_id);
			if(ptr_v>ptr_u)sched_kill_env(curenv->env_id);
			if(ptr_v+size>ptr_u)sched_kill_env(curenv->env_id);
		}
	case SYS_sbrk:
		return (uint32)sys_sbrk((int)a1);
		break;
	case SYS_free_user_mem:
		params_validation(a1,a2);
