...
//				cprintf("Expected Adrr =%x\n ",curVA + sizeOfMetaData());
//				cprintf("Actual Adrr =%x\n ",va);
//				cprintf("====================================================================\n");
				//Check returned va
				if(va == NULL || (va < curVA))
				{
	...
	/*Check memory allocation*/
	{
		...
		if ((freeFrames - sys_calculate_free_frames()) < (expectedAllocNumOfPages + expectedAllocNumOfTables))
		{
			...
			cprintf(" (expectedAllocNumOfPages + expectedAllocNumOfTables) = %d\n",(expectedAllocNumOfPages + expectedAllocNumOfTables));
